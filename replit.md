...
- **Auction System (UPDATED):** Time-based auction system supporting all item types with bidding mechanics, automatic settlement, and instant MongoDB saves. Uses clean sequential IDs (A001, A002, A003, etc.) for easy reference. **DUAL UI (Nov 27):** Supports both new form-based interface (click button → fill form with 5 fields) and classic text command (`!auction create <category> <item> <qty> <bid> [hours] [currency]`). Form fields: Category | Item Name | Quantity | Starting Bid | Duration (defaults to 24 hours). For gems currency, use text command.
