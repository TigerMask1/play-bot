...
- **Auction System (UPDATED):** Time-based auction system supporting all item types with bidding mechanics, automatic settlement, and instant MongoDB saves. Uses clean sequential IDs (A001, A002, A003, etc.) for easy reference. **DUAL UI (Nov 27):** Supports both new form-based interface (click button → fill form with all options) and classic text command (`!auction create <category> <item> <qty> <bid> [hours] [currency]`). Form properly handles optional fields (duration, currency) with sensible defaults.
