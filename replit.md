...
- **Auction System (UPDATED):** Time-based auction system supporting all item types with bidding mechanics, automatic settlement, and instant MongoDB saves. Uses clean sequential IDs (A001, A002, A003, etc.) for easy reference. **DUAL UI (Nov 27):** Multi-step form with category dropdown → item dropdown → modal (reduced typing errors). Classic text command also available: `!auction create <category> <item> <qty> <bid> [hours] [currency]`. Form defaults to coins, 24 hours. Text command supports gems currency.
